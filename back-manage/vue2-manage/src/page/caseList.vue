<template>
  ...
  <div class="fillcontain">
    ...
    <div class="table_container">
      ...
      <el-dialog title="修改案例信息" v-model="dialogFormVisible">
...
import { getCaseInfo, deleteCaseInfo, updateCaseInfo } from "@/api/getData";
export default {
  data() {
    return {
      offset: 0,
  ...
  methods: {
    ...
    async handleDelete(index, row) {
      try {
        const res = await deleteCase(row.item_id);
        if (res.status == 1) {
          this.$message({
            type: "success",
            message: "删除成功"
    ...
    async updateCase() {
      ...
      try {
        ...
        const res = await updateCaseInfo(postData);
        if (res.status == 1) {
          this.$message({
            type: "success",
            message: "更新信息成功"
